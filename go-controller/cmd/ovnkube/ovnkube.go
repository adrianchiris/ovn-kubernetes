...
func runOvnKube(ctx *cli.Context) error {
	...
	if config.Kubernetes.MetricsBindAddress != "" {
		if config.Kubernetes.OVNMetricsBindAddress == "" {
			...
			metrics.RegisterOvnMetrics(ovnClientset.KubeClient, master, config.MetricsScrapeInterval, stopChan)
		}
	...
	if config.Kubernetes.OVNMetricsBindAddress != "" {
		metrics.RegisterOvnMetrics(ovnClientset.KubeClient, node, config.MetricsScrapeInterval, stopChan)
