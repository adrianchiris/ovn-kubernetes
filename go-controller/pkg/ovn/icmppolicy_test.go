...
func (n icmpNetworkPolicy) addNamespaceSelectorCmds(fexec *ovntest.FakeExec, icmpNetworkPolicy *icmpnetworkpolicyapi.ICMPNetworkPolicy, findAgain bool) {
	...
	for i := range icmpNetworkPolicy.Spec.Ingress {
		fexec.AddFakeCmdsNoOutputNoError([]string{
			...
			"ovn-nbctl --timeout=15 --id=@acl create acl priority=1001 direction=to-lport match=\"ip4.src == {$a10424844185291987326} && outport == @a13918434151227952593\" action=allow-related log=false severity=info meter=acl-logging name=namespace1 external-ids:l4Match=\"None\" external-ids:ipblock_cidr=false external-ids:namespace=namespace1 external-ids:policy=icmp_networkpolicy1 external-ids:Ingress_num=0 external-ids:policy_type=Ingress -- add port_group " + readableGroupName + " acls @acl",
		})
		if findAgain {
			fexec.AddFakeCmdsNoOutputNoError([]string{
	...
	for i := range icmpNetworkPolicy.Spec.Egress {
		fexec.AddFakeCmdsNoOutputNoError([]string{
			...
			"ovn-nbctl --timeout=15 --id=@acl create acl priority=1001 direction=to-lport match=\"ip4.dst == {$a206675675430013742} && inport == @a13918434151227952593\" action=allow-related log=false severity=info meter=acl-logging name=namespace1 external-ids:l4Match=\"None\" external-ids:ipblock_cidr=false external-ids:namespace=namespace1 external-ids:policy=icmp_networkpolicy1 external-ids:Egress_num=0 external-ids:policy_type=Egress -- add port_group " + readableGroupName + " acls @acl",
		})
		if findAgain {
			fexec.AddFakeCmdsNoOutputNoError([]string{
		...
		ginkgo.It("correctly creates a icmpnetworkpolicy allowing a port to a local pod", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fExec.AddFakeCmdsNoOutputNoError([]string{
					...
					fmt.Sprintf("ovn-nbctl --timeout=15 --id=@acl create acl priority=1001 direction=to-lport match=\"ip4 && icmp4 && icmp4.type == %d && outport == @a13918434151227952593\" action=allow-related log=false severity=info meter=acl-logging name=%s external-ids:l4Match=\"icmp4 && icmp4.type == %d\" external-ids:ipblock_cidr=false external-ids:namespace=%s external-ids:policy=icmp_%s external-ids:Ingress_num=0 external-ids:policy_type=Ingress -- add port_group %s acls @acl", icmpType, icmpNetworkPolicy.Namespace, icmpType, icmpNetworkPolicy.Namespace, icmpNetworkPolicy.Name, readableGroupName),
					...
					fmt.Sprintf("ovn-nbctl --timeout=15 --id=@acl create acl priority=1001 direction=to-lport match=\"ip4 && icmp4 && icmp4.type == %d && inport == @a13918434151227952593\" action=allow-related log=false severity=info meter=acl-logging name=%s external-ids:l4Match=\"icmp4 && icmp4.type == %d\" external-ids:ipblock_cidr=false external-ids:namespace=%s external-ids:policy=icmp_%s external-ids:Egress_num=0 external-ids:policy_type=Egress -- add port_group %s acls @acl", icmpType, icmpNetworkPolicy.Namespace, icmpType, icmpNetworkPolicy.Namespace, icmpNetworkPolicy.Name, readableGroupName),
				})
